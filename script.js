document.addEventListener('DOMContentLoaded', () => {
    ...
    function initApp() {
        const targetDate = new Date('2026-06-06T00:00:00'); // Fecha de apertura (6 de Junio a medianoche)
        const now = new Date();
        ...
        if (now < targetDate) {
            ...
            overlay.className = 'screen-container';
            overlay.innerHTML = `
                <div class="container">
                    <h2 class="section-title">¡Falta muy poco! ⏳</h2>
                    <p class="hint">He preparado algo muy especial para ti, hermosa. Pero tendrás que ser paciente y esperar un poquito más. ¡Prometo que valdrá la pena! ❤️</p>
                    <div id="countdown-timer" style="margin-top: 30px;">
                        <div class="time-box"><span id="cd-days">0</span><p>Días</p></div>
                        <div class="time-box"><span id="cd-hours">0</span><p>Hrs</p></div>
                        <div class="time-box"><span id="cd-minutes">0</span><p>Min</p></div>
                        <div class="time-box"><span id="cd-seconds">0</span><p>Seg</p></div>
                    </div>
                </div>
            `;
            ...
            const timerInterval = setInterval(() => {
                ...
                document.getElementById('cd-minutes').textContent = String(Math.floor((diff % (1000 * 60 * 60)) / (1000 * 60))).padStart(2, '0');
                document.getElementById('cd-seconds').textContent = String(Math.floor((diff % (1000 * 60)) / 1000)).padStart(2, '0');
            }, 1000);
        } else {
            showPasswordScreen();
        }
